...
//                original.addAll(system.getSecurity().getHistory().getList().subList(first, last)); //old one
                //Esotelo enhancement
                List history = system.getSecurity().getHistory().getList();                
        ...
        protected IStatus run(IProgressMonitor monitor)
        {
            ...
            for (int i = 0; i < original.size(); i++)
            {
                ...
                history = new History();
//                history.addAll(original.getList().subList(0, i)); // old one
                // ESOTELO Bugfix
                List orginalHistory = original.getList();
                for(int j = 0; j < i+1; j++)
                {
                	history.add( (Bar)original.get(j) );
                }
                plugin.run();
